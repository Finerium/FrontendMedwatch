...
Claude Code should generate 50 patients with:
...
Here are 10 sample entries to show the pattern (Claude Code generates the remaining 40):
...
export const patients: Patient[] = [
  ...
  // ... Claude Code generates patient-011 through patient-050
...
Note: For the TopoJSON file, Claude Code should fetch Indonesia province boundaries from:
...
Claude Code should generate realistic 3D coordinates for these molecules. Approximate coordinates are fine since this is visual-only. Here are the molecules to model:
...
export const molecules: Molecule[] = [
  {
    ...
    atoms: [
      ...
      // Claude Code generates positions based on aspirin molecular geometry
...
// NOTE: Claude Code should generate actual [x, y, z] coordinates.
...
export const heatmapData: HeatmapData = {
  ...
  cells: [
    // Claude Code generates the full matrix (12 drugs x 12 side effects = 144 cells)
    ...
    // ... Claude Code fills in the remaining cells using drug side effect data
  ],
};
```
...
## Data Generation Notes for Claude Code
