    ...
    (async () => {
      try {
        ...
        if (data && data.drugs && data.drugs.length > 0 && data.effects && data.effects.length > 0) {
          setDrugs(data.drugs.slice(0, 7));
          setEffects(data.effects.slice(0, 7));
          if (Array.isArray(data.values) && data.values.length > 0) {
            const sliced = data.values
              .slice(0, 7)
              .map((row) => (Array.isArray(row) ? row.slice(0, 7) : []));
            setValues(sliced);
          }
        }
      } catch {}
  ...
  return (
    ...
    >
      ...
      <div className="stagger" style={{ position: "relative", zIndex: 2 }}>
        ...
        <div className="glass" style={{ padding: 28, overflowX: "auto", minHeight: 460 }}>
          {!loaded ? (
            <div
              className="skel"
